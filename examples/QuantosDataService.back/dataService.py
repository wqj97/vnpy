...
# ----------------------------------------------------------------------
def downMinuteBarBySymbol(api, vtSymbol, startDate, endDate='', freq='1M'):
    """下载某一合约的分钟线数据"""
    start = time()
    code, exchange = vtSymbol.split('.')
    if code in collections:
        print(u'合约{}已存在, 跳过下载'.format(code))
        return
    ...
    while dt <= end:
        ...
        df, msg = api.bar(symbol, freq=freq, trade_date=d)
        dt += delta
        if df is None:
            continue
...
def generateMounthCode(code):
    number = []
    year = 14
    month = 1
    temp_date = str(year) + str(month).zfill(2)
    endMounth = datetime.today().strftime('%y%m')
    code, exchange = code.split('.')
    while temp_date != endMounth:
        number.append(
            [str(year) + str(month).zfill(2) + '01', code + temp_date + '.' + exchange,
             str(year) + str(month).zfill(2) + str(calendar.monthrange(year, month)[1])])
        if month == 12:
            year += 1
            month = 1
        else:
    ...
    number.append(
        [str(year) + str(month).zfill(2) + '01', code + temp_date + '.' + exchange,
         str(year) + str(month).zfill(2) + str(datetime.today().day)])
    return number


# ----------------------------------------------------------------------
def downloadAllMinuteBar(api, freq='1M'):
    """下载所有配置中的合约的分钟线数据"""
    print '-' * 50
    print u'开始下载合约分钟线数据'
    print '-' * 50
    ...
    for symbol in SYMBOLS:
        for startDate, month_symbol, endDate in generateMounthCode(str(symbol)):
            downMinuteBarBySymbol(api, str(month_symbol), startDate, endDate, freq=freq)
